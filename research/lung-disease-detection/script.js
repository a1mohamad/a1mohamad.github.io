document.addEventListener("DOMContentLoaded", () => {
    ...
    const centerActiveLungTabOnMobile = (tab) => {
        if (!tab || !window.matchMedia('(max-width: 760px)').matches) return;
        const switcher = tab.closest('.notebook-switcher.lung-switcher');
        if (!switcher) return;
    ...
    notebookTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            centerActiveLungTabOnMobile(tab);
    ...
    document.querySelectorAll('.training-tabset').forEach(tabset => {
        ...
        const panels = shell ? shell.querySelectorAll('.training-model-panel') : [];
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                tab.classList.add('active');
                const targetPanel = shell ? shell.querySelector(`#${targetId}`) : null;
    ...
    document.querySelectorAll('.subsection-pill[data-model-target]').forEach(link => {
        link.addEventListener('click', (event) => {
            ...
            const trainingSection = document.getElementById('mobilenet-training');
            if (targetTab) targetTab.click();
            if (trainingSection) trainingSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
        });
    });
