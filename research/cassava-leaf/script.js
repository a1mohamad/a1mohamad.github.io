document.addEventListener("DOMContentLoaded", () => {
    ...
    blocks.forEach(block => {
        ...
        const increment = () => {
            ...
            if (currentStep < steps) {
                if (type === 'percentage') targetEl.innerText = initial.toFixed(2) + '%';
                else if (type === 'float') targetEl.innerText = initial.toFixed(4);
                else targetEl.innerText = Math.floor(initial);
                setTimeout(increment, runtime / steps);
            } else {
                if (type === 'percentage') targetEl.innerText = targetVal.toFixed(2) + '%';
                else if (type === 'float') targetEl.innerText = targetVal.toFixed(4);
                else targetEl.innerText = targetVal.toLocaleString();
            }
        };
        increment();
    });
    ...
    const panels = document.querySelectorAll('.notebook-panel');
    ...
    const refreshVisibleSectionHeights = () => {
        document.querySelectorAll('.section-collapsible').forEach(section => {
            if (section.offsetParent !== null) setBodyHeight(section);
        });
    };
    ...
    window.addEventListener('resize', refreshVisibleSectionHeights);
